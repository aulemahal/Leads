...
fid=fopen('Uinitk.bin','w','b'); fwrite(fid,Uinit,'real*8'); fclose(fid);
fid=fopen('Vinitk.bin','w','b'); fwrite(fid,Uinit,'real*8'); fclose(fid);
...
Xi = zeros(Nfloats);
Yj = zeros(Nfloats);
dxfloats = Lx/Nfloatsx;
dyfloats = Lx/Nfloatsy;

nn=1;
for ii=1:Nfloatsx
for jj=1:Nfloatsy
    Xi(nn) = (ii-1)*dxfloats+dxfloats/2;
    Yj(nn) = (jj-1)*dyfloats+dyfloats/2;
    nn=nn+1;
end
end
